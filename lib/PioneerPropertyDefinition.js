...
const propertyDefinition = {
	...
	"general.selectedInput": {
		...
		rx: { match: "^FN([0-9]{2})$", transform: (v) => parseInt(v[0]), update: ["audio.status.signal", "audio.dsp.*", "audio.toneControl.*"] },
	...
	"amp.hdmiAudio": {
		...
		rx: { match: "^HA([019])$", transform: (v) => parseInt(v[0]), update: ["audio.status.signal"] },
	...
	"audio.dsp.signalSource": {
		...
		rx: { match: "^SDA([0-39])$", transform: (v) => parseInt(v[0]), update: ["audio.status.signal"] },
	...
	"audio.dsp.virtualSurroundBack": {
		...
		rx: { match: "^VSB([0-1])$", transform: (v) => v[0] == "1", update: ["audio.status.signal"] },
		meta: { type: "boolean", role: "switch" }
	},
	...
	"audio.dsp.virtualHeight": {
		...
		rx: { match: "^VHT([0-1])$", transform: (v) => v[0] == "1", update: ["audio.status.signal"] },
		meta: { type: "boolean", role: "switch" }
	},
	...
	"audio.dsp.virtualDepth": {
		...
		rx: { match: "^VDP([0-3])$", transform: (v) => parseInt(v[0]), update: ["audio.status.signal"] },
	...
	"audio.listeningMode.selected": {
		...
		rx: { match: "^SR([0-9]{4})$", transform: (v) => v[0], update: ["audio.status.signal"] },
		meta: { type: "string", role: "variable", states: PioneerTypes.SelectedListeningMode }
	},
	"audio.listeningMode.current": {
		qy: { flags: "C", cmd: "?L" },
		rx: { match: "^LM([0-9a-fA-F]{4})$", transform: (v) => PioneerTypes.ListeningModes[v[0]], update: ["audio.status.signal"] },
		meta: { type: "string", role: "variable" }
	},
