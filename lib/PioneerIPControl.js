...
class IPControl extends EventEmitter {		
	...
	#processMessage = (m) => {
		...
		if (m.startsWith("GEP02020")) { this.#InternetRadioStation = m.substring(8, m.length - 9); this.emit("changed", "internetRadioStation"); res = true; }
		if (m.startsWith("GEP06029")) { this.#InternetRadioBitRate = m.substring(8, m.length - 9); this.emit("changed", "internetRadioBitRate"); res = true; }
		if (m.startsWith("GEP01032")) { this.#InternetRadioDescription = m.substring(8, m.length - 9); this.emit("changed", "internetRadioDescription"); res = true; }
		if (m.startsWith("SAB")) { 
