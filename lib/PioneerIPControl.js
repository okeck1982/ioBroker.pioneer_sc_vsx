...
class IPControl extends EventEmitter {
	 ...
	 * @returns {void}
	 */
	setConfig = (host, port, reconnectTimeout, keepAlive) => {	
	 ...
	 */
	 #queryProperty = (name) => {
		...
		if( propDef?.qy?.cmd && this.isPropertyEnabled(name) ) {
	...
	#query = (name=undefined) => {
		if(name) {
			...
			Object.getOwnPropertyNames(PPD).forEach(iName => {
				...
				if( propDef?.qy?.flags?.includes("C") ) {
	...
	#processSendProperty = (name, value) => {
		...
		if( prop.propDef?.tx?.transform && this.isPropertyEnabled(name) ) {
			// Process send Hook
			...
			if( isMatch ) {	
				...
				if( this.#sendCommand(prop.propDef.tx.transform(prop.value)) )
				{
					...
					if( prop.propDef?.qy?.flags?.includes("W") ) { this.#queryProperty(prop.name);}
					return true;
				}
			} else {
