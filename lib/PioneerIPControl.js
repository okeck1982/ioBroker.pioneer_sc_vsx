...
class IPControl extends EventEmitter {		
	...
	#processMessage(m) {
		var res = false;
		if (m == "") { return res; }
		if (m.startsWith("PWR")) { this.#power = (m.substr(3, 1) == "0"); this.emit("changed", "power"); }				
		if (m.startsWith("FN") ) { this.#selectedInput = m.substr(2, 2); this.emit("changed", "selectedInput"); res = true; }
		if (m.startsWith("HO") ) { this.#hdmiOutput = parseInt(m.substr(2, 1)); this.emit("changed", "hdmiOutput"); }
		if (m.startsWith("SPK")) { this.#speakerSelect = parseInt(m.substr(3, 1)); this.emit("changed", "speakerSelect"); }				
		if (m.startsWith("VOL")) { this.#volume = 12 - (0.5 * (185 - parseInt(m.substr(3)))); this.emit("changed", "volume"); }
		if (m.startsWith("MUT")) { this.#mute = (m.substr(3, 1) == "0"); this.emit("changed", "mute"); }								
		if (m.startsWith("HA") ) { this.#hdmiAudio = parseInt(m.substr(2, 1)); this.emit("changed", "hdmiAudio"); res = true; }
		if (m.startsWith("SDA")) { this.#signalSource = parseInt(m.substr(3, 1)); this.emit("changed", "signalSource"); res = true; }
		if (m.startsWith("SR") ) { this.#selectedListeningMode = m.substr(2, 4);	this.emit("changed", "selectedListeningMode"); res = true; }
		if (m.startsWith("FL") ) { this.#display = this.#decodeDisplayData(m.substr(4));	this.emit("changed", "display"); }
		if (m.startsWith("LM") ) { this.#currentListeningMode = PioneerTypes.ListeningModes[m.substr(2,4).toLowerCase()]; this.emit("changed", "currentListeningMode"); res = true; }
		if (m.startsWith("SAB")) { 
			this.#sleepTimer = parseInt(m.substr(3, 3)).toString();					
			clearTimeout(this.#sabRequestTimer);
		...
		if (m.startsWith("AST")) {
			let res = this.#decodeAST(m.substr(3));
			this.#inputSignal = res.codec;					
			this.#channelInputFormat = res.chInFormat;					
			this.#channelOutputFormat = res.chOutFormat;
	...
	#decodeDisplayData(data) {		
		...
		PioneerTypes.DisplayCharReplacements.forEach((i) => { 
			result = result.replace(i.s, i.r); 
		});
		return result;
	}
	...
	#decodeAST(data) {		
		...
		if(data) {
			...
			result.codec = PioneerTypes.AudioInputCodec[parseInt(data.substr(0, 2))] + " " + PioneerTypes.AudioInputFreq[parseInt(data.substr(2, 2))];
