...
class IPControl extends EventEmitter {		
	...
	#processMessage = (m) => {
		...
		if (m.startsWith("GEP02020")) { this.#InternetRadioStation = m.substring(9, m.length - 1); this.emit("changed", "internetRadioStation"); res = true; }
		if (m.startsWith("GEP06029")) { this.#InternetRadioBitRate = m.substring(9, m.length - 1); this.emit("changed", "internetRadioBitRate"); res = true; }
		if (m.startsWith("GEP01032")) { this.#InternetRadioDescription = m.substring(9, m.length - 1); this.emit("changed", "internetRadioDescription"); res = true; }
		this.log.info("command: " + m);
		if (m.startsWith("GIC049")) { this.#InternetRadioIcon = m.substring(7, m.length - 1); this.log.info("got icon: " + this.#InternetRadioIcon); this.emit("changed", "internetRadioIcon"); res = true; }
		if (m.startsWith("SAB")) { 
